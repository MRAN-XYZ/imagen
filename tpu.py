...
def load_dataset(root_path, image_size=64):
    ...
    for i, file_path in enumerate(files):
        try:
            ...
            img_array = np.array(img, dtype=np.uint8) / 127.5 - 1.0  # Normalize to [-1, 1]
...
def train_discriminator_step(d_state, g_state, real_batch, rng):
    ...
    d_state = d_state.replace(batch_stats=new_d_batch_stats)
    return d_state, loss
...
def train_generator_step(g_state, d_state, rng):
    ...
    def g_loss_fn(g_params):
        g_variables = {'params': g_params, 'batch_stats': g_state.batch_stats}
        noise = random.normal(rng, (batch_size, NZ, 1, 1))
        fake_batch, new_g_vars = g_state.apply_fn(g_variables, noise, training=True, mutable=['batch_stats'])
        d_variables = {'params': d_state.params, 'batch_stats': d_state.batch_stats}
        fake_logits = d_state.apply_fn(d_variables, fake_batch, training=False)
        real_labels = jnp.ones((batch_size, 1)) * 0.9  # Generator wants D to think fakes are real
        return bce_with_logits_loss(fake_logits, real_labels), new_g_vars['batch_stats']

    (loss, new_g_batch_stats), grads = value_and_grad(g_loss_fn, has_aux=True)(g_state.params)
    g_state = g_state.apply_gradients(grads=grads)
    g_state = g_state.replace(batch_stats=new_g_batch_stats)
    return g_state, loss
...
def generate_samples(g_state, noise):
    ...
    g_variables = {'params': g_state.params, 'batch_stats': g_state.batch_stats}
    return g_state.apply_fn(g_variables, noise, training=False)
...
class EMAState:
    def __init__(self, params, decay=0.999):
        self.decay = decay
        self.shadow = jax.tree.map(lambda x: x.copy(), params)
...
print(f"Generator parameters: {sum(x.size for x in jax.tree.leaves(g_state.params)):,}")
print(f"Discriminator parameters: {sum(x.size for x in jax.tree.leaves(d_state.params)):,}")
